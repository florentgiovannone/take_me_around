...
export function getTrackedArtworkUrl(path: string) {
  return `${ARKIN_PUBLIC_ORIGIN.replace(/\/$/, "")}${path}`
}

/** Reject other TMA hosts; allow path-only redirect messages and SEEN JSON payloads. */
function isArkinDomainMessage(message: string) {
  const normalized = message.toLowerCase()
  if (normalized.includes("takemearound.gallery") && !normalized.includes("arkin")) return false
  if (normalized.includes("takemearound.museum")) return false
  if (normalized.includes("http")) {
    return ARKIN_HOST_MARKERS.some((marker) => normalized.includes(marker))
  }
  return true
}

export function extractTrackedPathFromMessage(message: string): string | null {
  if (!isArkinDomainMessage(message)) return null

  const normalized = message.toLowerCase()

  for (const path of TRACKED_ARKIN_PATHS_BY_LENGTH) {
...
export type ArkinActivityEntry = {
  key: string
...
export function buildArkinActivityEntries(logs: PoiseLog[]): ArkinActivityEntry[] {
  const arkinLogs = getArkinLogs(logs)
  const redirects = arkinLogs.filter(
    (row) => normalizeMessageType(row.txt_message_type) === "REDIRECTED"
  )
  const seenLogs = arkinLogs.filter(
  ...
  const entries = Array.from(groups.values()).map((group) => {
    ...
    return {
      ...
      link: primary ? getArkinLogLink(primary) : "-",
    }
  })
...
export function buildTrackedArtworkScanGroups(logs: PoiseLog[]): TrackedArtworkScanGroup[] {
  ...
  return TRACKED_ARKIN_ARTWORKS.map((artwork) => ({
    ...artwork,
...
export function buildAudienceAnalytics(logs: PoiseLog[]) {
  const seen = arkinSeenEntries(logs)
  const scans = getRedirectScans(logs)
...
export function buildSarTimelineRowMetaMap(logs: PoiseLog[]): Map<string, SarTimelineRowMeta> {
  ...
  for (const sar of listDistinctArkinSars(logs)) {
    if (meta.has(sar)) continue
    const related = getArkinLogsForSar(logs, sar).sort((a, b) => {
      const aTime = parseLogTimestampGmt(a.dtm_timestamp)?.getTime() ?? 0
      const bTime = parseLogTimestampGmt(b.dtm_timestamp)?.getTime() ?? 0
      return bTime - aTime
    })
