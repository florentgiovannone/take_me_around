...
export const TRACKED_CHURCH_OF_ENGLAND_ARTWORKS = [
  ...
  { title: "The Kiss (Lovers)", path: "/the-kiss" },
] as const
...
export function getChurchOfEnglandLogs(logs: PoiseLog[]) {
  return logs.filter(isChurchOfEnglandLog)
}
...
function churchOfEnglandSeenEntries(logs: PoiseLog[]) {
  const churchOfEnglandTagNames = new Set<string>(
    TRACKED_CHURCH_OF_ENGLAND_ARTWORKS.map((artwork) => artwork.title)
  )
  ...
  return getSeenEntries(logs)
    ...
    .filter((entry): entry is ParsedSeen => {
      ...
      return churchOfEnglandTagNames.has(entry.textName.trim())
    })
}
...
function buildChurchOfEnglandSarByTagUid(logs: PoiseLog[]): Map<string, string> {
...
function resolveSarForChurchOfEnglandLog(log: PoiseLog, sarByUid: Map<string, string>): string | null {
...
export function listDistinctChurchOfEnglandSars(logs: PoiseLog[]): string[] {
  const sarByUid = buildChurchOfEnglandSarByTagUid(logs)
  const values = new Set<string>()
  for (const log of getChurchOfEnglandLogs(logs)) {
    const sar = resolveSarForChurchOfEnglandLog(log, sarByUid)
...
export function buildSarTimelineRowMetaMap(logs: PoiseLog[]): Map<string, SarTimelineRowMeta> {
  ...
  const seenCandidates = getChurchOfEnglandLogs(logs)
    ...
    .map((log) => ({
      log,
      sar: resolveSarForChurchOfEnglandLog(log, sarByUid),
