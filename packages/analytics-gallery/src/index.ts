...
export const TRACKED_GALLERY_ARTWORKS = [
  { title: "Florent Test", path: "/gallery-nouvion-4738593893849" },
  { title: "The Two Fridas", path: "/the-two-fridas" },
  { title: "Along the river", path: "/along-the-river-during-qingming-festival-by-zhang-zeduan-attributed-12th-century" },
...
const TRACKED_GALLERY_PATHS = TRACKED_GALLERY_ARTWORKS.map((artwork) => artwork.path)
const TRACKED_GALLERY_PATHS_BY_LENGTH = [...TRACKED_GALLERY_PATHS].sort(
  (a, b) => b.length - a.length
)
const TRACKED_GALLERY_TITLES = new Set(
  TRACKED_GALLERY_ARTWORKS.map((artwork) => artwork.title.toLowerCase())
)

/** NFC tag names that map to a canonical tracked gallery path. */
const TRACKED_GALLERY_NAME_TO_PATH = new Map<string, string>([
  ["the fallen madonna with the big boobies", "/gallery-nouvion-4738593893849"],
  ["along the river during qingming festival", "/along-the-river-during-qingming-festival-by-zhang-zeduan-attributed-12th-century"],
])

const TRACKED_GALLERY_SEEN_NAMES = new Set<string>([
  ...TRACKED_GALLERY_TITLES,
  ...TRACKED_GALLERY_NAME_TO_PATH.keys(),
])

/** Reject .museum and other hosts; allow path-only redirect messages and SEEN JSON payloads. */
function isGalleryDomainMessage(message: string) {
  const normalized = message.toLowerCase()
  if (normalized.includes("takemearound.museum")) return false
  if (normalized.includes("http") && !normalized.includes("takemearound.gallery")) {
    return false
  }
  return true
}
...
export function getTrackedArtworkUrl(path: string) {
  return `https://takemearound.gallery${path}`
}

export function extractTrackedPathFromMessage(message: string): string | null {
  if (!isGalleryDomainMessage(message)) return null

  const normalized = message.toLowerCase()

  for (const path of TRACKED_GALLERY_PATHS_BY_LENGTH) {
...
function nameMatchesTrackedGalleryArtwork(name: string | null | undefined) {
  const normalized = name?.trim().toLowerCase()
  if (!normalized) return false
  return TRACKED_GALLERY_TITLES.has(normalized) || TRACKED_GALLERY_NAME_TO_PATH.has(normalized)
}
...
export function resolveTrackedArtwork(log: PoiseLog): TrackedArtwork | null {
  ...
  if (path) {
    return TRACKED_GALLERY_ARTWORKS.find((artwork) => artwork.path === path) ?? null
  }

  const normalizedName = log.text_name?.trim().toLowerCase()
  if (!normalizedName) return null

  const aliasPath = TRACKED_GALLERY_NAME_TO_PATH.get(normalizedName)
  if (aliasPath) {
    return TRACKED_GALLERY_ARTWORKS.find((artwork) => artwork.path === aliasPath) ?? null
  }

  return (
    TRACKED_GALLERY_ARTWORKS.find((artwork) => artwork.title.toLowerCase() === normalizedName) ??
    null
  )
}
...
export function getGalleryLogLink(log: PoiseLog) {
...
function seenNeighborForGalleryRedirect(redirect: PoiseLog, byId: Map<number, PoiseLog>) {
...
/** Gallery logs plus nearby SEEN rows Poise writes next to a gallery REDIRECTED (same tag name, or unnamed). */
export function getGalleryLogs(logs: PoiseLog[]) {
  const galleryLogs = logs.filter(isGalleryLog)
  const included = new Set(galleryLogs.map((log) => log.int_id))
...
export type GalleryActivityEntry = {
  key: string
...
export function buildGalleryActivityEntries(logs: PoiseLog[]): GalleryActivityEntry[] {
  const galleryLogs = getGalleryLogs(logs)
  const redirects = galleryLogs.filter(
    (row) => normalizeMessageType(row.txt_message_type) === "REDIRECTED"
  )
  const seenLogs = galleryLogs.filter(
  ...
  const entries = Array.from(groups.values()).map((group) => {
    ...
    return {
      ...
      link: primary ? getGalleryLogLink(primary) : "-",
    }
  })
...
export function buildTrackedArtworkScanGroups(logs: PoiseLog[]): TrackedArtworkScanGroup[] {
  ...
  const sortByTimestampDesc = (a: PoiseLog, b: PoiseLog) => {
    const aTime = parseTimestamp(a.dtm_timestamp)?.getTime() ?? 0
    const bTime = parseTimestamp(b.dtm_timestamp)?.getTime() ?? 0
    return bTime - aTime
  }

  return TRACKED_GALLERY_ARTWORKS.map((artwork) => ({
    ...artwork,
...
function hasExplicitTimezoneForLocalParse(s: string) {
  return hasExplicitTimezone(s) || /T\d/.test(s)
}

export function parseLogTimestamp(value: string | null | undefined): Date | null {
  if (value == null) return null
  const trimmed = String(value).trim()
  if (!trimmed) return null

  const utcNaive =
    typeof import.meta.env.VITE_DASHBOARD_TIMESTAMPS_UTC === "string" &&
    import.meta.env.VITE_DASHBOARD_TIMESTAMPS_UTC === "true"
  ...
  if (dayOnly && trimmed.length <= 12) {
    ...
    const mo = Number(dayOnly[2])
    const dom = Number(dayOnly[3])
    const d = new Date(y, mo - 1, dom, 0, 0, 0, 0)
    return Number.isNaN(d.getTime()) ? null : d
  }

  if (!hasExplicitTimezoneForLocalParse(trimmed)) {
    const m = NAIVE_ISO_LOCAL.exec(trimmed)
    if (m) {
      const hour = Number(m[4])
      const minute = Number(m[5])
      ...
      if (
        ...
        const dom = Number(m[3])
        const frac = m[7] != null ? `.${m[7]}` : ""
        let iso = `${y}-${String(mo).padStart(2, "0")}-${String(dom).padStart(2, "0")}T${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}:${String(sec).padStart(2, "0")}${frac}`
        if (utcNaive) iso += "Z"
        const d = new Date(iso)
        if (!Number.isNaN(d.getTime())) return d
      }
...
function parseTimestamp(value: string | null) {
  return parseLogTimestamp(value)
}
...
function buildGallerySarByTagUid(logs: PoiseLog[]): Map<string, string> {
  const map = new Map<string, string>()
  for (const log of getGalleryLogs(logs)) {
...
function resolveSarForGalleryLog(log: PoiseLog, sarByUid: Map<string, string>): string | null {
...
/** Distinct SAR values seen on tracked .gallery links (from SEEN cookies, linked to redirects by tag UID). */
export function listDistinctGallerySars(logs: PoiseLog[]): string[] {
  const sarByUid = buildGallerySarByTagUid(logs)
  const values = new Set<string>()
  for (const log of getGalleryLogs(logs)) {
    const sar = resolveSarForGalleryLog(log, sarByUid)
...
export function buildSarTimelineRowMetaMap(logs: PoiseLog[]): Map<string, SarTimelineRowMeta> {
  ...
  const seenCandidates = getGalleryLogs(logs)
    .filter((log) => (log.txt_message_type ?? "").trim().toUpperCase() === "SEEN")
    .map((log) => ({
      log,
      sar: resolveSarForGalleryLog(log, sarByUid),
  ...
  for (const sar of listDistinctGallerySars(logs)) {
    if (meta.has(sar)) continue
    const related = getGalleryLogsForSar(logs, sar).sort((a, b) => {
      const aTime = parseLogTimestampGmt(a.dtm_timestamp)?.getTime() ?? 0
      const bTime = parseLogTimestampGmt(b.dtm_timestamp)?.getTime() ?? 0
      return bTime - aTime
    })
...
/** All tracked .gallery log rows for one SAR (case-insensitive). */
export function getGalleryLogsForSar(logs: PoiseLog[], sarQuery: string): PoiseLog[] {
  const target = sarQuery.trim().toLowerCase()
  if (!target) return []
  const sarByUid = buildGallerySarByTagUid(logs)
  return getGalleryLogs(logs).filter((log) => {
    const sar = resolveSarForGalleryLog(log, sarByUid)
    return sar?.toLowerCase() === target
  })
}

export type SarGalleryTimelineEvent = GalleryActivityEntry

export function buildSarGalleryTimelineEvents(
  logs: PoiseLog[],
  sarQuery: string
): GalleryActivityEntry[] {
  if (!sarQuery.trim()) return []
  return buildGalleryActivityEntries(getGalleryLogsForSar(logs, sarQuery))
}

/** Visible window: centre to each edge = this duration (1 hour per half = 2h span default). */
export const SAR_TIMELINE_VIEWPORT_HALF_MS = 60 * 60 * 1000

/** Min zoom: 5 minute total span (2.5 min each side of now). */
export const SAR_TIMELINE_ZOOM_MIN_HALF_MS = 2.5 * 60 * 1000

/** Max zoom: 2 hour total span (1 hour each side of now). */
export const SAR_TIMELINE_ZOOM_MAX_HALF_MS = 60 * 60 * 1000

/** Discrete zoom steps (half-window ms); index 0 = most zoomed in. No 40 min window. */
...
/** Default grid step at max zoom out (2 hour window). */
export const SAR_TIMELINE_GRID_STEP_MS = 30 * 60 * 1000

/** Max grid/tick DOM nodes across the full scroll width. */
export const SAR_TIMELINE_MAX_GRID_TICKS = 72

/** Tick label interval for each fixed zoom window (by half-window ms). */
...
/** Visible time range (+ buffer) for virtualising ticks and markers while scrolling. */
...
function gallerySeenEntries(logs: PoiseLog[]) {
  return getSeenEntries(logs)
    .map(parseSeenEntry)
    .filter((entry): entry is ParsedSeen => {
      if (!entry?.textName?.trim()) return false
      const normalized = entry.textName.trim().toLowerCase()
      return TRACKED_GALLERY_SEEN_NAMES.has(normalized)
    })
}
...
export function buildOverviewAnalytics(logs: PoiseLog[]) {
  ...
  const scansByTag = scans.reduce<Record<string, number>>((acc, row) => {
    const label = resolveTrackedArtwork(row)?.title || row.text_name?.trim() || "Unknown"
...
export function buildAudienceAnalytics(logs: PoiseLog[]) {
  const seen = gallerySeenEntries(logs)
  const scans = getRedirectScans(logs)
