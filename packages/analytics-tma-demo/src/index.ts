...
function isAnonymousSeenLog(log: PoiseLog) {
  return (
    normalizeMessageType(log.txt_message_type) === "SEEN" &&
    !log.text_name?.trim() &&
    !log.txt_uid?.trim() &&
    (log.txt_message ?? "").trim().startsWith("{")
  )
}
...
function anonymousSeenNeighborForTmaDemoRedirect(redirect: PoiseLog, byId: Map<number, PoiseLog>) {
  for (const neighborId of [redirect.int_id - 1, redirect.int_id + 1]) {
    ...
    if (!isAnonymousSeenLog(neighbor)) continue
    if (!timestampsCloseEnough(neighbor, redirect)) continue
    return neighbor
  }
  return null
}
